    ...
    let _inithub = {
        ...
        status: {
            ...
            doing: {
                //正在做什么
                scan: 2,
                //0:芯片0扫描;1:芯片1扫描;2代表停止扫描
                node: ''//正在连接设备的mac
            }
        },
        connetedPeripherals: {
            //已连接设备
            checkConnTime: hubConfig.connetedPeripherals.checkConnTime,
            //检查连接时间周期
            CheckConnTimeExp: 0,
            //时间后检查
            Peripherals: {
...
let hubs = {
    ...
    scan(mac, chip) {
        chip = chip || 0
        debugger const hub = this.hubs[mac]
        if (!this.__online(mac)) {
            return
        }
        this.__es(hub, 'scan', hub.info.server + '/gap/nodes/?event=1&mac=' + mac + '&chip=' + chip + '&access_token=' + hub.info.access_token, function(event) {
    ...
    notify(mac) {
        ...
        if (!hub.output.notify) {
            this.__es(hub,'notify', hub.info.server + '/gatt/nodes/?event=1&mac=' + hub.info.mac + '&access_token=' + hub.info.access_token, function(event) {
    ...
    __clearOldLoac() {
        for (let node in this.locationData) {
            if (this.locationData[node].life === 0) {
    ...
    __slectHubByName(name, option) {
        ...
        let result = {}, allScanData = [], temp, len2 = Object.keys(filtedData).length, resultNode = []
    ...
    __scanDataColl(o) {
        ...
        function addData(node, hub) {
            ...
            hub.scanData.origin[node].life = 4
            //生命周期是4秒
            hub.scanData.origin[node].times = len
            hub.scanData.origin[node].free = true

            if (!this.locationData[node]) {
                this.locationData[node] = hub.scanData.origin[node]
            } else {
                if (this.locationData[node].avg < hub.scanData.origin[node].avg) {
                    this.locationData[node] = hub.scanData.origin[node]
                }
            }
    ...
    conn(o) {
        ...
        if (!mac && node) {
            this.__slectHubByNode(node)
        } else if (mac && node && type) {
            this.__conn(o)
        } else if (name && !mac) {
            this.__autoConn(name)
        }
