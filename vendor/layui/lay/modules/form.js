...
layui.define('layer', function (exports) {
    ...
    var submit = function () {
        var button = $(this), verify = form.config.verify, stop = null
            ...
            /**
            * wang
            * 只选择 lay-select 属性的表单元素
            */
            , select = button.attr('lay-select')
            , fieldElem = null
            , verifyElem = null //获取需要校验的元素
            , formElem = button.parents('form')[0] //获取当前所在的form元素，如果存在的话
            , filter = button.attr('lay-filter'); //获取过滤器

        //获取所有表单域
        if (select) {
            fieldElem = elem.find('input['+select+'],select['+select+'],textarea['+select+']')
            verifyElem = elem.find('*[lay-verify]['+select+']')
        } else {
